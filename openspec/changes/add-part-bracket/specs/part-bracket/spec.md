...
- **WHEN** 零件有側面接口（如 USB/接線口）且開啟擋牆
- **THEN** 擋牆在對應牆面、接口位置開出缺口，使接口不被擋牆擋住
...
- **THEN** 生成垂直背板（零件安裝孔對應的鎖附孔）＋水平底座（含底座鎖附孔），零件直立鎖在背板上；底座向後的長度可由 `baseDepth` 指定（未設定時用 `baseMargin`），底座厚度為 `baseThickness`
