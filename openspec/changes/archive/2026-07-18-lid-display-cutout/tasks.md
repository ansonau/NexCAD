...
- [x] 5.2 瀏覽器驗證：放 OLED 0.96 產生螺絲上蓋外殼，上蓋出現螢幕視窗開孔、位置對齊螢幕
- [x] 5.3 瀏覽器驗證：關閉選項重新產生，開孔消失；重新開啟恢復
- [x] 5.4 瀏覽器驗證：無顯示器零件（如 Uno 單獨）上蓋無窗（確認）；旋轉 90° 後窗跟著轉，此案例 UI 無旋轉輸入欄位，改由 `portProjection.test.ts` 單元測試涵蓋（0°/90°/45° 跳過三案例皆綠燈）
